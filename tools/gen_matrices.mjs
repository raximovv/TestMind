...
// The artwork, split out so it can be fetched when challenge 6 opens
// rather than on every first load. See the note above OUT_ART below.
const OUT_ART = path.join(process.cwd(), 'assets', 'matrix-art.js')
...
var MATRIX_ITEMS = ${JSON.stringify(items.map(({ id, rules, difficulty, answer }) =>
    ({ id, rules, difficulty, answer })), null, 2)}
...
const art = `// GENERATED by tools/gen_matrices.mjs -- do not edit by hand.
//
// The tiles for challenge 6, keyed by item id: the 3x3 grid with its corner
// missing, and the eight options in the order they are shown. Kept apart from
// matrix.js because this is 66 KB of SVG and nothing needs it until a student
// opens the challenge -- matrix.js carries the answers and the count, which is
// all the hub has to know.
//
// Every path is drawn in currentColor, so the tiles follow the page's text
// colour instead of carrying a palette of their own.

var MATRIX_ART = ${JSON.stringify(Object.fromEntries(
    items.map((i) => [i.id, { grid: i.grid, options: i.options }])), null, 2)}
`

fs.writeFileSync(OUT, body, 'utf8')
fs.writeFileSync(OUT_ART, art, 'utf8')
console.log(`wrote ${path.relative(process.cwd(), OUT_ART)}  ${(art.length / 1024).toFixed(0)} KB of tiles`)
